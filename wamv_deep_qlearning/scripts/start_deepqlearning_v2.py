...
class DQNRobotSolver():
    def __init__(self, env, n_observations, n_actions, n_win_ticks=195, min_episodes= 100, max_env_steps=None, gamma=1.0, epsilon=1.0, epsilon_min=0.01, epsilon_log_decay=0.995, alpha=0.01, alpha_decay=0.01, batch_size=64, monitor=False, quiet=False):
        self.memory = deque(maxlen=100000)
        self._env = env
        #if monitor: self._env = gym.wrappers.Monitor(self._env, '../data/turtlebot2-1', force=True)
        ...
        self.quiet = quiet
        if max_env_steps is not None: self._env._max_episode_steps = max_env_steps
...
if __name__ == '__main__':
    rospy.init_node('wamv_nav_deep_qlearn', anonymous=True, log_level=rospy.FATAL)
    ...
    rospackage_name = "wamv_deep_qlearning"
    model_name = "wamv_nav"
    
    environment_name = 'WamvNavTwoSetsBuoys-v0'
    
    n_observations = rospy.get_param('/wamv/n_observations')
    n_actions = rospy.get_param('/wamv/n_actions')
    
    n_episodes_training = rospy.get_param('/wamv/episodes_training')
    n_episodes_running = rospy.get_param('/wamv/episodes_running')
    n_win_ticks = rospy.get_param('/wamv/n_win_ticks')
    min_episodes = rospy.get_param('/wamv/min_episodes')
    max_env_steps = None
    gamma =  rospy.get_param('/wamv/gamma')
    epsilon = rospy.get_param('/wamv/epsilon')
    epsilon_min = rospy.get_param('/wamv/epsilon_min')
    epsilon_log_decay = rospy.get_param('/wamv/epsilon_decay')
    alpha = rospy.get_param('/wamv/alpha')
    alpha_decay = rospy.get_param('/wamv/alpha_decay')
    batch_size = rospy.get_param('/wamv/batch_size')
    monitor = rospy.get_param('/wamv/monitor')
    quiet = rospy.get_param('/wamv/quiet')
